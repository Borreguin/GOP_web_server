...
key = b'\xbf\xc0\x85)\x10nc\x98\x02)j\xdf\xcb\xc4\x98\x9d(\x9e[EX\xc8\xd5\xbfI{\xa2$\x05(\xd5\x18'
...
def encrypt_file(file_name, key=key):
...
def decrypt_file(file_name, key=key):
    with open(file_name, 'rb') as fo:
        ciphertext = fo.read()
...
def encrypt_tag_obj(obj_w_tag):
    ...
    if isinstance(obj_w_tag, dict):
        if 'tag' in obj_w_tag.keys():
            obj_w_tag['tag'] = encrypt(obj_w_tag['tag'], key)
            return obj_w_tag

    if isinstance(obj_w_tag, list):
        for d in obj_w_tag:
            if 'tag' in d.keys():
                d['tag'] = encrypt(d['tag'], key)
        return obj_w_tag
...
def decrypt_tag_obj(obj_w_tag):
    ...
    if isinstance(obj_w_tag, dict):
        if 'tag' in obj_w_tag.keys():
            obj_w_tag.tag = decrypt(obj_w_tag.tag, key)

    if isinstance(obj_w_tag, list):
        for d in obj_w_tag:
            if 'tag' in d.keys():
                d['tag'] = decrypt(d['tag'], key)
        return obj_w_tag
